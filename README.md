...
Find the generated plan at `plan/masterplan.json`. A standalone HTML report (`plan/masterplan.html`) is emitted so you can visually review counts, charts, filters, and diffs in any browser without touching the JSON. Apply mode will read the JSON file and dispatch each item to the relevant service (sides, transaction types, derived portfolios, portfolio groups, chart of accounts, accounts, posting rules, ABOR configurations, or ABORs).
