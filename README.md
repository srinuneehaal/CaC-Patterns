...
Find the generated plan at `plan/masterplan.json`. A standalone HTML report (`plan/masterplan.html`) is emitted by default so you can visually review counts, charts, filters, and diffs in any browser without touching the JSON. Set `MASTER_PLAN_REPORT_ENABLED=false` (or `0`/`off`/`no`) to skip writing the HTML. Apply mode will read the JSON file and dispatch each item to the relevant service (sides, transaction types, derived portfolios, portfolio groups, chart of accounts, accounts, posting rules, ABOR configurations, or ABORs).
